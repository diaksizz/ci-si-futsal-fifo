...
function pesan($data)
{
    ...
    // Periksa apakah ada pemesanan lain yang tumpang tindih
    $check_query = "SELECT COUNT(*) as count FROM sewa_212279 WHERE 212279_id_lapangan = '$idlpg' AND ('$mulai' < 212279_jam_habis AND '$habis' > 212279_jam_mulai)";
    $result = mysqli_query($conn, $check_query);
    $row = mysqli_fetch_assoc($result);

    if ($row['count'] > 0) {
        // Jika ada konflik waktu, tampilkan pesan error
        echo "<script>
              alert('Error: Sudah booked');
              document.location.href = 'lapangan.php';
              </script>";
        exit;
    } else {
        // Jika tidak ada konflik waktu, tambahkan data baru
        $query = "INSERT INTO sewa_212279 (212279_id_user, 212279_id_lapangan, 212279_lama_sewa, 212279_jam_mulai, 212279_jam_habis, 212279_harga, 212279_total) VALUES ('$userid','$idlpg','$lama','$mulai','$habis','$harga','$total')";
        ...
        if (mysqli_query($conn, $query)) {
            ...
            // Tampilkan pesan error dengan JavaScript alert dan alihkan halaman
            echo "<script>
